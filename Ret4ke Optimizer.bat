...
if %errorlevel% neq 0 (
echo                Ret4ke Optimizer needs to run as Administrator
powershell -NoProfile -NonInteractive -Command start -verb runas "'%~s0'" >nul 2>&1
if !errorlevel! equ 0 exit /b
echo.
echo             ERROR: Optimizer isn't running as Administator.
echo                              Continue anyway?
echo.
choice /c:"YN" /n /m "%BS%               [Y] Yes  [N] No" & if !errorlevel! equ 2 exit /b
)
